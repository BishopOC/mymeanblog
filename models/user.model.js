...
userSchema.methods.validPassword = function(password){
  const hash = crypto.pbkdf2Sync(password, this.salt, 1000, 64, 'sha512')
                     .toString('hex');
  return this.hash === hash; //if they equal, ye shall pass, otherwise yooouu shaaall nooot paaaaaaaaassss
};
userSchema.methods.generateJwt = function(){
  const expiration = new Date();
  expiration.setDate(expiration.getDate() + 7); //creates a date that is 7 days in the future
  return jwt.sign({
    _id: this._id,
    email: this.email,
    name: this.name,
